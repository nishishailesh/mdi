...
class astm_file(file_mgmt,my_sql):
  ...
  def manage_soh(self,data):
    print_to_log('SOH(hex 01)', ' detected')
  ...
  def on_any_line(self,any_line):
    ...
    print_to_log('on_any_line(): exploded line is: ',temp)    
    return tuple(temp) 
  ...
  def on_patient(self,patient_line):
    ...
    print_to_log('Now self.result is',self.result)
  ...
  def on_query(self,query_line):
    ...
    print_to_log('Now self.result is:{}'.format(self.result))
  ...
  def on_termination(self,termination_line):
    ...
    #removed, because, sample_id less data is managed somewhere else
    if len(str(self.sample_id))>0:
      self.final_data=self.final_data + ((self.sample_id,self.result),)
    else:
      print_to_log('sample_id is blank',' but saving result')
      self.final_data=self.final_data + ((self.sample_id,self.result),)
