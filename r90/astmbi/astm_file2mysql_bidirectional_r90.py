...
from astm_file2mysql_bidirectional_general_for_r90 import astm_file
...
class astm_file_xl1000(astm_file):
  ...
  def manage_final_data(self):
    print_to_log('========manage_final_data()==========','========start============')
    print_to_log('manage_final_data(): final_data',self.final_data)
    print_to_log('========manage_final_data()==========','=======final data  received as tuple are as follows=============')
    ...
    for each_sample in self.final_data:
      # O record is not used by us. It is just finding sample_id, but, that work is done in general script while preparing tuple
      #('O', '1', '22435', 'Sample #^7982', '', '', '', '', '', '', 'ANONYMOUS', '', '', '', '', 'Arterial^', '', '', '', '', '', '', '', '', '', 'F')
      ...
      for each_record in each_sample[1]:
        ...
        if(each_record[0]=='R'):
          ...
          #####Unique ID code
            ...
            real_sample_id=self.find_sample_id_for_unique_id(con,query_sample_id) #this is number
          ...
          if(len(each_record)>=12):
            ...
            uniq=uniq
          print_to_log('final uniq string created is',uniq)
