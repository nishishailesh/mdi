...
class astm_file(file_mgmt,my_sql):
  ...
  def on_order(self,order_line):
    ...
    try:
      logging.debug("order_tuple[3]:{}".format(order_tuple[3]))
      self.sample_id=order_tuple[3]	#tuple 3  is for sysmax Godhra
      pstr='New Sample Id:({sid})'.format(sid=self.sample_id)
      logging.debug(pstr)
    except Exception as my_ex:
      pstr='Look at O record <<<{}>>> Is it inappropriate? no order_tuple[3] found'.format(order_tuple)
      logging.debug(pstr)
      logging.debug(my_ex)
