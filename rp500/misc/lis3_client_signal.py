...
while True:
  ...
  while s!=None:
    try:
      s.connect((HOST, PORT))
      print_to_log("connected:","ok....")
    except Exception as my_ex:
      print_to_log('can not connect to RP500:',my_ex)
      time.sleep(1)
      continue
    while True:
      signal.alarm(0)
      ...
      try:
        data = s.recv(3024)
        print_to_log('RECV:<<<', data)
        all_list=analyse_data(data)
      except Exception as my_ex:
        print_to_log("ERROR:>>>>>",my_ex)
        break; 
      try:
        signal.alarm(0)
        signal.alarm(10)
      ...
      if(data_type==b'SMP_NEW_AV'):
        ...
        print_to_log("rSEQ AVAILABLE==========>",find_rSEQ(all_list))
        pre_smp_req= STX+b'SMP_REQ' +FS+RS+b'aMOD'+GS+b'0500'+GS+GS+GS+FS+b'iIID'+GS+b'53726'+GS+GS+GS+FS+b'rSEQ'+GS+rSEQ+GS+GS+GS+FS+RS+ETX
