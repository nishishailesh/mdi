...
class astm_file(file_mgmt,my_sql):
  ...
  def analyse_file(self):
    ...
    print_to_log('analyse_file(): self.relevant_data',self.relevant_data)
  ...
  def mk_tuple(self):
    ...
    print_to_log('mk_tuple() raw_data',raw_data)
    each_line=raw_data.split('\x0d')
    print_to_log('mk_tuple() each_line',each_line)
    ...
    print_to_log('mk_tuple() self.final_data',self.final_data)
  ...
  def on_header(self,header_line):
    ...
    header_tuple=self.on_any_line(header_line)  #No use of this variable
  ...
  def on_patient(self,patient_line):
    ...
    if(len(self.result)>0):           #first data is empty
      print_to_log('on_patient() self.result:',self.result)
      self.final_data=self.final_data + ((self.result),)  #no decision on sample id just now
  ...
  def on_order(self,order_line):
    ...
    try:
      ...
      pstr='on_order() New Sample Id:({sid})'.format(sid=self.sample_id)
      logging.debug(pstr)
    except Exception as my_ex:
      pstr='on_order() Look at O record <<<{}>>> Is it inappropriate? no order_tuple[2] found'.format(order_tuple)
      logging.debug(pstr)
      logging.debug(my_ex)
