...
class astm_file_xl1000(astm_file):
  ...
  def manage_final_data(self):
    #print_to_log('manage_final_data:','returning without doing anything. A step in development')    
    #return
    ...
    for each_sample in self.final_data:
      print_to_log('manage_final_data() each_sample:',each_sample)
      
      for each_record in each_sample:
        if(each_record[0]=='P'):
          if(conf.sample_id_record=='patient'):
            query_sample_id=each_record[3]
            print_to_log('manage_final_data() conf.conf.sample_id_record=patient so, query_sample_id=',query_sample_id)
        if(each_record[0]=='O'):
          if(conf.sample_id_record=='order'):
            query_sample_id=each_record[3]
            print_to_log('manage_final_data() conf.conf.sample_id_record=order so, query_sample_id=',query_sample_id)
        if(each_record[0]=='R'):
          print_to_log('checking if query query_sample_id is ready. It is:',query_sample_id)
          ...
          #Example R tuple of quikread
          #R tuple: ('R', '1', '^^^CRP', '18', 'mg/l', '', '', '', '', '', '', '', '20231124200000', 'QuikRead go A17047I61827')
