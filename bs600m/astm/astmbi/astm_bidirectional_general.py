...
class astms(astmg.astmg, file_mgmt):
  ...
  def manage_read(self,data):
    ...
    #for receiving data
    data=self.prev_data+data	#add previsous daa to current data, if data is not full frame e.g stx-etx-lf etc (for Mindray BS600M)
    print_to_log('Prev_data:',self.prev_data)
    print_to_log('Final DATA in manage_read after prev_data:',data)
    if(data==b'\x05'):
      self.prev_data=b''
      signal.alarm(0)
      self.main_status=1
      ...
      print_to_log("byte_data",self.byte_data)

      #if( (self.send_status==1 or self.send_status==2) and (self.current_line<self.total_lines or self.current_line==0 ) ): for vitros
      if( (self.send_status==1 or self.send_status==2)  ):
        ...
        self.byte_data=fd.read(4096)
        #print_to_log('byte_data',self.byte_data)

        self.write_msg=self.byte_data #set message
        self.byte_data=b'' #no need to maintain it	
      ...
      #self.prev_data not zeroed
      self.prev_data=data		#for next cycle, however, overwritten every where except incomplate frame
  ###################################
  #override this function in subclass
  ###################################
