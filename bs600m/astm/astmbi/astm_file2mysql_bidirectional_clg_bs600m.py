...
class astm_file_xl1000(astm_file):
  ...
  def make_order(self,query_sample_id,real_sample_id,requested_examination_code):
    ...
    #ex_code_str=b''
    #for one_code in requested_examination_code:
    #  ex_code_str=ex_code_str+requested_examination_code.encode()+three_caret_byte
    ex_code_str='^^^\\'.join(requested_examination_code).encode("UTF-8")+b'^^^\\'
    ...
    header_line=  b'1H'+self.s1.encode()+self.s2.encode()+self.s3.encode()+self.s4.encode()+b'|||Mindray|||||||SA|1394-97|'+time_now.encode()+b'\x0d'
    patient_line= b'2P|1|NOPID|||NONAME^^|||F'+b'\x0d'
    #order_line=   b'3O|1|'+sample_id.encode()+b'||'+ex_code_str+b'|R||'+time_now.encode()+b'||||N||||5||||||||||O'+b'\x0d'
    order_line=   b'3O|1|'+query_sample_id.encode()+b'||'+ex_code_str+b'|R||'+time_now.encode()+b'||||N||||'+vitros_sample_code+b'||||||||||O'+b'\x0d'
    terminator_line=b'4L|1|N'+b'\x0d'

    return self.format_astm_message(header_line+patient_line+order_line+terminator_line)

  #for vitros, name changed because this file is for bs600m
  def format_astm_message_for_vitros(self,message): 
  ...
  def format_astm_message(self,message):
    message_for_checksum=b'\x02'+message+b'\x03'
    message_checksum=self.get_checksum(message_for_checksum)
    print_to_log('Calculated checksum=: ',message_checksum)
    final_message=message_for_checksum+message_checksum+b'\x0d\x0a'
    print_to_log('Final message: ',final_message)
