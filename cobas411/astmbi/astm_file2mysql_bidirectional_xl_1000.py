...
class astm_file_xl1000(astm_file):
    ...
    for each_sample in self.final_data:
      ...
      for each_record in each_sample[1]:
        ...
        if(each_record[0]=='Q'):
          ...
          #for Cobas411
          #Q|1|^^25364298^156^0^2^^S1^SC||ALL||||||||A
          sample_id_list=each_sample[0].split(self.s3+self.s3)[1].split(self.s3)[0].split(self.s2)
          print_to_log('sample_id_list:',sample_id_list)

          for query_sample_id in sample_id_list:
            print_to_log('query_sample_id.rstrip(' ')...',query_sample_id)
            query_sample_id=query_sample_id.rstrip(' ')
            print_to_log('query_sample_id.isnumeric()???',query_sample_id)
            #####Unique ID code
            if(query_sample_id.isnumeric() == True):
              real_sample_id=query_sample_id
            else:
            ...
            print_to_log('Q tuple[2] :',each_record[2])
            sample_id_area=each_record[2]
            print_to_log('Q tuple sample_id_area:',sample_id_area)
            additional_str='^'.join(sample_id_area.split('^')[3:])
            print_to_log('Q tuple additional_str:',additional_str)
            ...
            print_to_log('=====>Single Q Data:',single_q_data)
            ...
            #direct readymade str passed for cobas 411
            order_to_send=self.make_order(query_sample_id,ex_code_str)
  ...
  def make_order(self,sample_id,requested_examination_code):
    '''
    Cobas 411 cobas type (not elecsys type)
    
    1H|\^&|||cobas-e411^1|||||host|TSREQ^REAL|P|1
    Q|1|^^50310083636960211^172^0^6^^S1^SC||ALL||||||||A  
    L|1|N
    01

    ^^50310083636960211       ^172 ^0     ^6    ^   ^S1     ^SC
    ^^________sample_id       ^seq ^car   ^pos  ^   ^s type ^cont type

    ^B1H|\^&|||cl_general
    P|1|||||||
    O|1|50310083636960211|^^^`|R||||||N||||serum
                              ^seq  ^carr ^pos  ^   ^s type      ^con type
    L|1N
    ^C28

    '''
    ##cobas411 readymade ex_code_str passed, not tuple
    ##ex_code_str=b'^^^'+'^'.join(requested_examination_code).encode()
    
    
    ex_code_str=requested_examination_code
